...
movesController.mostRecentUsage = async (req, res, next) => {
    console.log('Inside movesController.mostRecentUsage')
    try {
        ...
        if (num === undefined) {
            ...
            if (num > 150) return next({ message: 'Error inside movesController.filterUsageData. Requested num too big' });
        ...
        const log = `Error occuring in movesController.mostRecentUsage: ${err}`;
...
movesController.specificPokemonUsage = async (req, res, next) => {
    // console.log('Inside movesController.specificPokemonUsage')
    try {
        ...
        const log = `Error occuring in movesController.specificPokemonUsage: ${err}`;
        const message = { err: 'Error occured on server side' };
        return next({ log: log, message: message });
    }
